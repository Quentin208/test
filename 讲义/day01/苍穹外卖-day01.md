...
==**此处思考，前后端为什么请求地址的端口号不同？还能连接上？**==

**答案：**

​	因为这里采用了NGINX反向代理，前端请求的是NGINX的服务端口号，后端接受的NGINX的服务端口号，NGINX相当于是中间商，服务两边
...
- 进行负载均衡（**负责把任务合理分配给各个服务器**）

  所谓负载均衡,就是把大量的请求按照我们指定的方式均衡的分配给集群中的每台服务器。

- 保证后端服务安全（**前端不直接访问后端服务器**）
...
2. 修改Java代码，前端提交的密码进行MD5加密后再跟数据库中密码比对（**md5加密是单向的，无法反向解密，所有只能对比加密后的字符串是否相等**）
...
将课程资料中提供的项目接口导入YApi。访问地址：[YApi Pro-高效、易用、功能强大的可视化接口管理平台](https://yapi.pro/group/372121)
...
目前，一般都使用knife4j框架（**简化了swagger的使用难度**）。
...
4. 访问测试
   ...
   **这个页面的login和logout是通过解析以下文件中的方法（类）而来的**
   
   ![image-20240729111431946](C:\Users\lenovo\AppData\Roaming\Typora\typora-user-images\image-20240729111431946.png)
